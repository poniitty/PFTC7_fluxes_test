...
# filename <- "raw_data/Site 1/1_2000_east_1_night_resp.txt"
flux_calc_own <- function (fluxfiles, param = "nee", skip = 3, vol = 2.197, area = 1.69) {
  ...
  if (length(fluxfiles$photo_names) >= 1 & length(fluxfiles$resp_names) >= 1) {
    ...
    if (length(fluxfiles$resp_names) >= 1) {
      stats.df <- purrr::map_df(sort(fluxfiles$resp_names), ~neeet.fit(.))
    }
    if (length(fluxfiles$photo_names) >= 1) {
      stats.df <- purrr::map_df(sort(fluxfiles$photo_names), ~neeet.fit(.))
    }
